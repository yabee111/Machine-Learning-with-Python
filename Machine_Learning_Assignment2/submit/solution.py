...
def backward_selection(X, y, pipe, nr_remove=5):
    ...
    #For the evaluation, use 3-fold shuffled crossvalidation and r2 score
    sf = ShuffleSplit(n_splits=3, test_size=0.1, train_size=0.4, random_state=0)

    removed_features = []
    
    # if the feature is important will have larger influence when being removed, means the cv score will be low
    # so choosing the highest score when which feature is removed means has least influence
    for j in range(0, nr_remove):
        scores = []
        #remove each feature and compute the score      
        for i in all_features:
            x = X_sub.drop(i, axis=1)
            clf = pipe(x, knn)
            score = cross_validate(clf, X, y, scoring='r2', cv=sf, return_estimator =True)
            scores.append([score['test_score'].mean(), i])
        all_features = all_features.drop(max(scores)[1])
        X_sub = X_sub.drop(max(scores)[1], axis=1)
        removed_features.append(max(scores)[1])
...
# count how many features are still being constructed by your simple_pipeline
categorical = X.select_dtypes(include=["category", "object"]).columns.tolist()
numerical = X.select_dtypes(exclude=["category","object"]).columns.tolist()

feature_numbers = 0
for i in categorical:
    feature_numbers = feature_numbers + len(X[i].value_counts().to_dict())
feature_numbers = feature_numbers + len(numerical)
...
def plot_2_2(X, y):
    ...
    models = [Ridge(random_state=0), Lasso(random_state=0,max_iter=10000), RandomForestRegressor(random_state=0), GradientBoostingRegressor(random_state=0)]
    encoders = [OneHotEncoder(sparse=False, handle_unknown='ignore'), OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1), TargetEncoder(handle_missing='value')]
    
    #shuffle data for cross validation, 5-fold
    sf = ShuffleSplit(n_splits=5, random_state=0)


    all_columns = [model.__class__.__name__ for model in models]
    all_rows = [encoder.__class__.__name__ for encoder in encoders]
    scores = []
    
    #5-fold cross validation with r2 score 
    for encoder in encoders:
        for model in models:
            clf = flexible_pipeline(X, model = model, encoder = encoder)
            x_scores = cross_validate(clf, X, y, scoring='r2', cv=sf)
            scores.append(np.mean(x_scores['test_score']))
            
    #reshape for heatmap
    scores = np.reshape(scores, (3, 4))
    heatmap(all_columns, all_rows, scores)
...
def create_embeddings(X, y):
    ...
    X_copy = X.copy()
    categorical = X_copy.select_dtypes(include=["category"]).columns.tolist()

    # change category to strings
    for i in categorical:
        X_copy[i] = X_copy[i].astype(str)

    sup_vec = SuperVectorizer(auto_cast = True, impute_missing='auto')
    X_transform = sup_vec.fit_transform(X_copy, y)

    X_transformed_dataframe = pd.DataFrame(X_transform, columns = sup_vec.get_feature_names())

    return X_transformed_dataframe
    pass
...
def compute_tsne(X):
    ...
    tsne = TSNE(n_components = 2).fit_transform(X.values)
    return tsne
    pass
...
def plot_tsne(tsne_embeds, scores):
    ...
    plt.scatter(tsne_embeds.T[0], tsne_embeds.T[1], c=scores)
    plt.show()
    pass
...
def plot_2_5(X, y):
    ...
    #covert category to object
    for i in categorical:
        X_copy[i] = X_copy[i].astype(str)
    
    #same shuffle and cv evaluation with Question2.2
    sf = ShuffleSplit(n_splits=5, random_state=0)
    sup_vec = SuperVectorizer(auto_cast = True, low_card_cat_transformer = OneHotEncoder(handle_unknown='ignore'))
    
    #for plot
    all_columns = [model.__class__.__name__ for model in models]
    all_rows = ['SuperVectorizer']
    scores = []

    for model in models:
        x_scores = cross_validate(flexible_pipeline(X_copy, model = model, encoder = sup_vec), X_copy, y, scoring='r2', cv=sf)
        scores.append(np.mean(x_scores['test_score']))
    
    scores = np.reshape(scores, (1,4))
    
    return heatmap(all_columns, all_rows, scores)
    pass
# plot_2_5(X, y)
...
def plot_3_1(X, y):
    ...
    # Pass the tree-based and permutation importances to the plotting function compare_importances below.
    return compare_importances(rf_feature_importances, permutation_importances, rf_feature_names, X.columns)
    pass
...
def plot_4_1(X, y):
    ...
    rf = RandomForestRegressor(n_estimators=100, max_features=0.1, random_state=0)
    clf = flexible_pipeline(X, rf)
    scores = cross_val_predict(clf, X, y, cv=3, method='predict')
    
    # seperate the predictions by gender
    gender_M = np.where(X['gender']=='M')
    gender_F = np.where(X['gender']=='F')
    
    gender_M_pred = [scores[i] for i in gender_M]
    gender_F_pred = [scores[i] for i in gender_F]

    gender_M_true = [y[i] for i in gender_M]    
    gender_F_true = [y[i] for i in gender_F]

    r2_score_M = r2_score(gender_M_true[0], pd.Series(gender_M_pred[0]))
    r2_score_F = r2_score(gender_F_true[0], pd.Series(gender_F_pred[0]))
    
    #plot a bar chart
    fig, ax = plt.subplots(figsize=(4, 4))
    x_axis = ['M', 'F']
    y_axis = [r2_score_M, r2_score_F]
    ax.bar(x_axis, y_axis, color = 'green', width = 0.3)
...
def plot_4_2(X, y):
    ...
    gender_weights = X['gender'].value_counts()/len(y)
    sample_weights = [gender_weights[r if r is not np.nan else 'F'] for r in X['gender']]
    
    rf = RandomForestRegressor(n_estimators=100, max_features=0.1, random_state=0)
    clf = flexible_pipeline(X, rf)

    scores = cross_val_predict(clf, X, y, cv=3, method='predict', fit_params = {'model__sample_weight': sample_weights})
